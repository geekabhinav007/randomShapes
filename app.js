...
var shapes = ['circle', 'rectangle', 'triangle', `pentagon`, `ploygon`, `star`, `heart`];
...
// Returns a random time between 100 and 800 milliseconds
function getRandomTime() {
  return randomInRange(100, 800);
}
...
function drawShape() {
  ...
  if (shape === 'circle') {
    ...
    var pentagon = two.makePolygon(x, y, size, 5);
    pentagon.fill = color;
  }else if (shape === 'ploygon') {
    var ploygon = two.makePolygon(x, y, size, 6);
    ploygon.fill = color;
  }else if (shape === 'star') {
    var star = two.makeStar(x, y, size, 5, 0.5);
    star.fill = color;
  }else if (shape === 'heart') {
    var heart = two.makeHeart(x, y, size);
    heart.fill = color;
  }
  ...
  if(time > getRandomTime() && time < getRandomTime()+50) {
    two.clear();
    setTimeout(drawShape, time);
  }
}
